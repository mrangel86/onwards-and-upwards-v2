...
const BlogCard: React.FC<BlogCardProps> = ({
  ...
  >
    ...
    <div className="flex flex-col gap-1 py-4 px-5 flex-1">
      <h3 className="font-playfair font-bold text-lg text-primary mb-1 line-clamp-2 h-14">{title}</h3>
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs text-accent">by {author}</span>
        {date && <span className="text-xs text-gray-500">{date}</span>}
      </div>
      <p className="text-gray-700 text-sm leading-snug line-clamp-3 h-14">{excerpt}</p>
    </div>
  </div>
);
